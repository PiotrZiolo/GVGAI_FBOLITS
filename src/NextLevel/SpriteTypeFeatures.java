...
public class SpriteTypeFeatures
{
	 ...
	 * 
	 * @param weights
	 *            array of weights used to calculate partial score.
	 * @param distanceToSprite
	 *            distance from current position to given sprite.
	 */
	public double getScore(double[] weights, double distanceToSprite)
	{
		...
		double spriteScore = filledWeights[0] * this.dangerousToAvatar
				...
				+ filledWeights[2] * ((this.destroyable) ? 1 : 0) + filledWeights[3] * ((this.collectable) ? 1 : 0)
				+ filledWeights[4] * ((this.givingVictory) ? 1 : 0) + filledWeights[5] * ((this.givingDefeat) ? 1 : 0)
				+ filledWeights[6] * this.givingPoints + filledWeights[7] * ((this.passable) ? 1 : 0)
				+ filledWeights[8] * ((this.moving) ? 1 : 0) + filledWeights[9] * this.speed
