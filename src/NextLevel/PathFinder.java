...
public class PathFinder {
	...
	int[] toDirection(Types.ACTIONS act){
		if(act==Types.ACTIONS.ACTION_UP) return new int[]{0,-1};
		if(act==Types.ACTIONS.ACTION_DOWN) return new int[]{0,1};
		if(act==Types.ACTIONS.ACTION_LEFT) return new int[]{1,0};
		else return new int[]{-1,0};
	}
	
	Types.ACTIONS getDirection(int[] s, int[] g){
		if(s[0]<g[0])return Types.ACTIONS.ACTION_LEFT;
		if(s[0]>g[0])return Types.ACTIONS.ACTION_RIGHT;
		if(s[1]<g[1])return Types.ACTIONS.ACTION_UP;
	...
	Deque<Types.ACTIONS> pathFinder(Vector2d s, Vector2d g, StateObservationMulti stateObs, int index){
		...
		//those are the vectors in a "divided by blckSze" metric, had to initialize both
		int[] start = new int[0];
		int[] goal = new int[]{(int)g.x/size, (int)g.y/size};
		
		//if the positions_objects is true, then it's a pessimistic position, so we can approximate to it
		boolean none = true;
		for(int i=0;i<positions.size();i++)
			if(positions_objects[i]){
				none=false;
		...
		Types.ACTIONS[] act = {Types.ACTIONS.ACTION_RIGHT, Types.ACTIONS.ACTION_LEFT, Types.ACTIONS.ACTION_UP,Types.ACTIONS.ACTION_DOWN};
