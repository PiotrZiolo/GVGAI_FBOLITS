...
public class PathFinder
{
	...
	public PathFinder(GameKnowledge gameKnowledge)
	{
	...
	Deque<Types.ACTIONS> findPath(Vector2d from, Vector2d to, StateObservationMulti stateObs, int playerID)
	{
		...
		positions.add(new int[] { (int) from.x / size, (int) from.y / size });
		if (from.x % size != 0)
		{
		...
		// initialize both
		int[] start = new int[0];
		int[] goal = new int[] { (int) to.x / size, (int) to.y / size };
		...
		for (Types.ACTIONS i : stateObs.getAvailableActions(playerID))
